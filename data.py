...
class Animals:
    def get_data(self):
        ...
        def split_sex_upon_outcome():
                data['sex']= data.sex_upon_outcome.apply(lambda x : x.split(" ")[0])
                data['sex_type']= data.sex_upon_outcome.apply(lambda x : x.split(" ")[-1])
                return data['sex'], data['sex_type']
